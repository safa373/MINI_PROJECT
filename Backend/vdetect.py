...
#
#                     mediapath="C:\\Users\\user\\PycharmProjects\\violencedetection\\media\\"
#
#                     for i in lkframes:
#
...
#                         cv2.imwrite(ms,i)
#
#                         qry="INSERT INTO `myapp_incident_sub_images` (`image`,`VIOLENCE_id`) VALUES ('"+"/media/"+ filename +"','"+str(id)+"')"
#
#                         db.insert(qry)
#
#                 lkframes=[]
...
def video_mamonreader(cv2,filename):
    db = Database()
    qryq = "SELECT * FROM `myapp_student` "
    resq = db.select(qryq)
    ...
    for i in resq:
        s = i["photo"]
        s = s.replace("/media/", "")
        pth = "C:\\Users\\user\\PycharmProjects\\violencedetection\\media\\"+s

        # pth = "C:\\Users\\Acer\\PycharmProjects\\AI_CCTV_ideal\\media\\" + s
                        ...
                        for i in range(0, totface):
                            ...
                            for j in res:
                                if j == True:
                                    ...
                                    qryChk = "SELECT * FROM `myapp_violencesub` where `STUDENT_id`='"+str(knownids[i])+"' and `VIOLENCE_id`='"+str(id)+"'"
                                    ...
                                    if len(resChk)>0:
                                        ...
                                        # qry = "INSERT INTO `myapp_violencehub` (`STUDENT_id`,`VIOLENCE_id`) VALUES ('" + str(knownids[i]) + "','" + str(id) + "')"
                                        qry = "INSERT INTO `myapp_violencehub`(`action`,`STUDENT_id`,`VIOLENCE_id`) VALUES ('pending','" + str(knownids[i]) + "','" + str(id) + "')"
                                        # db.insert(qry)
