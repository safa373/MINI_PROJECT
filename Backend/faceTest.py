...
while(True):
    ...
    cv2.imwrite(r"C:\\Users\\user\\PycharmProjects\\violencedetection\\media\\tests\\b.jpg",frame)
    ...
    for i in range(0,totface):
        ...
        for j in res:
            if j==True:
                ...
                qry="SELECT * FROM `myapp_attendance` WHERE `date`=CURDATE() and checkin_checkout='Check in' and STUDENT_id='"+str(knownids[l])+"'"
                resa=db.selectOne(qry)

                if resa is not None:

                    qry="update myapp_attendance set  checkin_checkout='Check out' where STUDENT_id='"+str(knownids[l])+"'"
                    db.update(qry)
                else:


                    qry="INSERT INTO `myapp_attendance` (`checkin_checkout`,`date`,`STUDENT_id`,`time`,`photo`) VALUES ('Check in',CURDATE(),'"+str(knownids[l])+"',CURTIME(),'')"
                    db.insert(qry)
            l=l+1
vid.release()
